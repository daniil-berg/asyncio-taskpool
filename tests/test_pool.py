...
class BaseTaskPoolTestCase(CommonTestCase):
    ...
    def test_num_running(self) -> None:
        self.task_pool._tasks_running = {1: MagicMock(), 2: MagicMock(), 3: MagicMock()}
        self.assertEqual(3, self.task_pool.num_running)

    def test_num_cancelled(self) -> None:
        self.task_pool._tasks_cancelled = {1: MagicMock(), 2: MagicMock(), 3: MagicMock()}
        self.assertEqual(3, self.task_pool.num_cancelled)

    def test_num_ended(self) -> None:
        self.task_pool._tasks_ended = {1: MagicMock(), 2: MagicMock(), 3: MagicMock()}
    ...
    async def test__check_start(self) -> None:
        ...
        try:
            ...
            self.task_pool._check_start(awaitable=mock_coroutine, function=None, ignore_lock=True)
        finally:
    ...
    async def test__task_cancellation(self, mock__task_name: MagicMock, mock_execute_optional: AsyncMock) -> None:
    ...
    @patch.object(pool, 'execute_optional')
    @patch.object(pool.BaseTaskPool, '_task_name', return_value=FOO)
    async def test__task_ending(self, mock__task_name: MagicMock, mock_execute_optional: AsyncMock) -> None:
        task_id, mock_task, mock_callback = 1, MagicMock(), MagicMock()
    ...
    async def test__task_wrapper(self, mock__task_name: MagicMock,
                                 mock__task_cancellation: AsyncMock, mock__task_ending: AsyncMock) -> None:
        task_id = 42
    ...
    async def test__start_task(self, mock__check_start: MagicMock, mock_reg_cls: MagicMock, mock__task_name: MagicMock,
                               mock__task_wrapper: AsyncMock, mock_create_task: MagicMock) -> None:
    ...
    def test_cancel(self, mock__get_running_task: MagicMock, mock__get_cancel_kw: MagicMock) -> None:
    ...
    def test__cancel_and_remove_all_from_group(self, mock__cancel_group_meta_tasks: MagicMock) -> None:
        kw = {BAR: 10, BAZ: 20}
        task_id = 555
        ...
        class MockRegister(Set[Any], MagicMock):
            pass
        self.task_pool._cancel_and_remove_all_from_group(' ', MockRegister({task_id, 'x'}), **kw)
        mock_cancel.assert_called_once_with(**kw)

    @patch.object(pool.BaseTaskPool, '_get_cancel_kw')
    @patch.object(pool.BaseTaskPool, '_cancel_and_remove_all_from_group')
    def test_cancel_group(self, mock__cancel_and_remove_all_from_group: MagicMock, mock__get_cancel_kw: MagicMock) -> None:
        mock__get_cancel_kw.return_value = fake_cancel_kw = {'a': 10, 'b': 20}
    ...
    @patch.object(pool.BaseTaskPool, '_get_cancel_kw')
    @patch.object(pool.BaseTaskPool, '_cancel_and_remove_all_from_group')
    def test_cancel_all(self, mock__cancel_and_remove_all_from_group: MagicMock, mock__get_cancel_kw: MagicMock) -> None:
        mock__get_cancel_kw.return_value = fake_cancel_kw = {'a': 10, 'b': 20}
...
class TaskPoolTestCase(CommonTestCase):
    ...
    async def test__apply_spawner(self, mock__start_task: AsyncMock) -> None:
        ...
        await self.task_pool._apply_spawner(grp_name, mock_func, args, kw, num, end_cb, cancel_cb)
        ...
        await self.task_pool._apply_spawner(grp_name, mock_func, args, None, num, end_cb, cancel_cb)
    ...
    def test_apply(self, mock__check_start: MagicMock, mock__generate_group_name: MagicMock,
                   mock_reg_cls: MagicMock, mock__apply_spawner: MagicMock, mock_create_task: MagicMock) -> None:
    ...
    @patch.object(pool, 'execute_optional')
    async def test__get_map_end_callback(self, mock_execute_optional: AsyncMock) -> None:
    ...
    async def test__queue_consumer(self, mock_semaphore_cls: MagicMock, mock__get_map_end_callback: MagicMock,
                                   mock__start_task: AsyncMock, mock_star_function: MagicMock) -> None:
        n = 2
        ...
        end_cb, cancel_cb = MagicMock(), MagicMock()
        await self.task_pool._arg_consumer(grp_name, n, mock_func, args, 0, end_cb, cancel_cb)
        ...
        await self.task_pool._arg_consumer(grp_name, n, mock_func, arg_it, 1, end_cb, cancel_cb)
        mock_semaphore_cls.assert_called_once_with(n)
    ...
    def test__map(self, mock__check_start: MagicMock, mock_reg_cls: MagicMock, mock__arg_consumer: MagicMock,
        ...
        mock__arg_consumer.assert_called_once_with(group_name, n, func, arg_iter, 1,
                                                   end_callback=end_cb, cancel_callback=cancel_cb)
    ...
    @patch.object(pool.TaskPool, '_generate_group_name')
    def test_map(self, mock__generate_group_name: MagicMock, mock__map: MagicMock) -> None:
    ...
    @patch.object(pool.TaskPool, '_map')
    @patch.object(pool.TaskPool, '_generate_group_name')
    def test_starmap(self, mock__generate_group_name: MagicMock, mock__map: MagicMock) -> None:
        mock__generate_group_name.return_value = generated_name = 'name 1 2 3'
        mock_func = MagicMock()
    ...
    @patch.object(pool.TaskPool, '_map')
    @patch.object(pool.TaskPool, '_generate_group_name')
    async def test_doublestarmap(self, mock__generate_group_name: MagicMock, mock__map: MagicMock) -> None:
        mock__generate_group_name.return_value = generated_name = 'name 1 2 3'
        mock_func = MagicMock()
...
class SimpleTaskPoolTestCase(CommonTestCase):
    ...
    def test_stop(self, mock_cancel: MagicMock) -> None:
        num = 2
        id1, id2, id3 = 5, 6, 7
        self.task_pool._tasks_running = {id1: MagicMock(), id2: MagicMock(), id3: MagicMock()}
    ...
    def test_stop_all(self, mock_stop: MagicMock, mock_num_running: MagicMock) -> None:
