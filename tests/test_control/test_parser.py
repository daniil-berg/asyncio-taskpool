...
from argparse import ArgumentParser, ArgumentTypeError, HelpFormatter, ArgumentDefaultsHelpFormatter, RawTextHelpFormatter, SUPPRESS
...
class ControlParserTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.parser = parser.ControlParser(stream=self.stream, terminal_width=self.terminal_width, formatter_class=FOO)
    ...
    def test_add_function_command(self, mock_get_first_doc_line: MagicMock) -> None:
        def foo_bar() -> None: pass
    ...
    @patch.object(parser, 'get_first_doc_line')
    def test_add_property_command(self, mock_get_first_doc_line: MagicMock) -> None:
        def get_prop(_self: Any) -> None: pass
        def set_prop(_self: Any, _value: Any) -> None: pass
    ...
    def test_add_class_commands(self, mock_add_function_command: MagicMock, mock_add_property_command: MagicMock) -> None:
        class FooBar:
            some_attribute = None

            def _protected(self, _: Any) -> None: pass

            def __private(self, _: Any) -> None: pass

            def to_omit(self, _: Any) -> None: pass

            def method(self, _: Any) -> None: pass

            @property
            def prop(self) -> None: return None
        ...
        x = 'x'
        common_kwargs = {'stream': self.parser._stream, CLIENT_INFO.TERMINAL_WIDTH: self.parser._terminal_width}
    ...
    def test_add_function_arg(self, mock_add_argument: MagicMock, mock__get_type_from_annotation: MagicMock) -> None:
        ...
        def func(foo: Tuple[Any, ...], *args: str, bar: int = bar_default, baz: float = baz_default,
                 boo: complex = boo_default, flag: bool = False) -> Any:
...
class RestTestCase(TestCase):
    ...
    def test__get_arg_type_wrapper(self) -> None:
        ...
        name = 'abcdef'
        mock_type = MagicMock(side_effect=[ArgumentTypeError, TypeError, ValueError, Exception], __name__=name)
    ...
    def test__get_type_from_annotation(self, mock__get_arg_type_wrapper: MagicMock) -> None:
        ...
        for a1 in dotted_path_ann:
            self.assertEqual(expected_output, parser._get_type_from_annotation(a1))
        ...
        for a2 in literal_eval_ann:
            self.assertEqual(expected_output, parser._get_type_from_annotation(a2))
