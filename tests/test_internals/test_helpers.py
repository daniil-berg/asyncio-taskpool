...
from asyncio_taskpool.internals import constants, helpers
...
class HelpersTestCase(IsolatedAsyncioTestCase):
    ...
    def test_resolve_dotted_path(self) -> None:
        ...
        self.assertEqual(
            WARNING, helpers.resolve_dotted_path("logging.WARNING")
        )
        self.assertEqual(
            urlopen, helpers.resolve_dotted_path("urllib.request.urlopen")
        )
        with patch.object(
            helpers, "import_module", return_value=object
...
class ClassMethodWorkaroundTestCase(TestCase):
    def test_init(self) -> None:
        def func(_cls: object) -> str:
            return "foo"

        def getter(_cls: object) -> str:
            return "bar"
